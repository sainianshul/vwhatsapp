...
{
    /**
     * Sanitize a string to valid UTF-8 to prevent json_encode failures.
     */
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('message_text', function (WhatsAppMessage $msg) {
                $text = e($this->sanitizeUtf8($msg->message_text));
                if (mb_strlen($text) > 40) {
                    $truncated = mb_substr($text, 0, 40) . '...';
            ...
            ->editColumn('status', function (WhatsAppMessage $msg) {
                if ($msg->status === 'sent') {
                    ...
                    $errorMsg = e($this->sanitizeUtf8($msg->error_message ?? 'No error details available'));
